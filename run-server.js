...
```path/to/prolly last/vera-ventures/run-server.js
...
function sendHtmlResponse(res, statusCode, title, body, isLoggedIn) {
  sendResponse(res, statusCode, 'text/html', serveHtmlTemplate(title, body, isLoggedIn));
}
...
const routes = {
  ...
  '/': (req, res) => {
    ...
    let body = `
      ...
      <p>Join our community of entrepreneurs and builders who are shaping the future.</p>

      <div class="features">
        <div class="feature">
          <h3>Connect</h3>
          <p>Network with like-minded founders and investors in our exclusive community.</p>
        </div>
        <div class="feature">
          <h3>Learn</h3>
          <p>Access resources, mentorship, and insights from industry experts.</p>
        </div>
        <div class="feature">
          <h3>Grow</h3>
          <p>Get funding opportunities and support to scale your startup.</p>
        </div>
      </div>
      ...
      <div class="cta">
        ' + (isLoggedIn
            ...
            : '<h2>Ready to get started?</h2><button class="btn-block" onclick="window.location.href=\'/auth/signup\'">Sign Up Now</button>') + '
      </div>
    `;
  ...
  '/landing': (req, res) => {
    ...
    let body = `
      ...
      <div class="features">
        <div class="feature">
          <h3>Connect with Founders</h3>
          <p>Network with ambitious founders who share your vision and drive.</p>
        </div>
        <div class="feature">
          <h3>Access Resources</h3>
          <p>Get exclusive access to funding opportunities, mentorship, and industry insights.</p>
        </div>
        <div class="feature">
          <h3>Build Your Future</h3>
          <p>Join events, workshops, and programs designed to accelerate your growth.</p>
        </div>
      </div>
      ...
      <div class="cta">
        ...
        ' + (isLoggedIn
          ? '<button class="btn-block" onclick="window.location.href=\'/\'">Go to Dashboard</button>'
          : '<button class="btn-block" onclick="window.location.href=\'/auth/signup\'">Sign Up Now</button>') + '
      </div>
    `;
      ...
      <script>
        document.getElementById('forgotForm').addEventListener('submit', async (e) => {
          ...
          try {
            ...
            if (response.ok) {
              messageDiv.innerHTML = '<div class="success">Password reset link sent to your email!</div>';
            } else {
              messageDiv.innerHTML = '<div class="error">' + data.error + '</div>';
            }
          } catch (error) {
  ...
  '/auth/logout': (req, res) => {
    const session = getSessionFromRequest(req);
    if (!session) {
      return sendRedirect(res, '/auth/login');
    }

    // For simplicity, we'll just clear the session cookie and redirect
  ...
  '/api/auth/forgot-password': async (req, res) => {
    if (req.method !== 'POST') {
      return sendJsonResponse(res, 405, { error: 'Method not allowed' });
    }
    ...
    req.on('end', () => {
      try {
        const { email } = parseFormData(body);
